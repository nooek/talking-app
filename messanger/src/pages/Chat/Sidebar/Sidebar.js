    ...
    if (friendSearchName.length === 0) {
      ...
      if (newContact.length) {
        setFriends([
          ...friends,
          {
            friend_id: newContact[0].friend_id,
            friend_name: newContact[0].friend_name,
          },
        ]);
      }
    }
  }, [messages, userData, friends, friendSearchName]);
      ...
      <FriendsContainer>
        ...
        {friends.map((each, index) => {
          return (
            ...
            >
              {each.friend_pfp ? <FriendPfp src={each.friend_pfp} /> : null}
